    ...
    try {
        ...
        if (!accountNumber || !amount || transferAmount <= 0) {
            return res.render('customer/transfer', {
                user: req.user,
                account: await Account.findOne({ userId: req.user._id }),
        ...
        const updatedAccount = await Account.findOne({ userId: req.user._id });
        res.render('customer/transfer', {
            user: req.user,
            account: updatedAccount,
            error: null,
            success: `Transfer successful! ₹${transferAmount.toLocaleString()} sent to account ${accountNumber}`
        });
